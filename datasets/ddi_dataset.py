...
class DDIDataset(Dataset):
    def __init__(self, dataset_name, split, fold):
        file_path = f"data/{dataset_name}/{dataset_name}_{split}_{fold}.json"
...
class BatchLoader:
    ...
    def gen_drug_batch(self, drug_list):
        ...
        for drug in drug_list:
            ...
            data = Data(x= graph["x"], edge_index=graph["edge_index"], edge_attr=graph['edge_attr'], num_part=graph['num_part'],size_node=graph['size_node'], size_edge=graph['size_edge'], size_atom=graph['size_atom'],size_bond=graph['size_bond'])
