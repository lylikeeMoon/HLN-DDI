...
class DDIDataset(Dataset):
    def __init__(self, dataset_name, split, fold):
        file_path = f"data/{dataset_name}/{dataset_name}_ind_{split}_{fold}.json"
...
class BatchLoader:
    def __init__(self, args):
        ...
        self.graphs = torch.load(f"data/{dataset_name}/{dataset_name}_graphs.pt")
        self.nearest_old = self.get_nearest_old()
    
    def get_nearest_old(self):
        sim_mat_path = f"data/{self.dataset_name}/{self.dataset_name}_tanimoto.pt"
        sim_mat = torch.load(sim_mat_path).to(self.device)
        
        old_new_path = f"data/{self.dataset_name}/{self.dataset_name}_old_new_{self.fold}.json"
        with open(old_new_path, "r") as f:
            old_idx = json.load(f)["old"]
        
        old_idx = torch.LongTensor(old_idx).to(self.device)
        # (num_old_drugs, )
    ...
    def gen_drug_batch_train(self, drug_list):
        ...
        for drug in drug_list:
            ...
            data = Data(x=graph["x"], edge_index=graph["edge_index"], edge_attr=graph['edge_attr'],
                        num_part=graph['num_part'], size_node=graph['size_node'], size_edge=graph['size_edge'],
                        size_atom=graph['size_atom'], size_bond=graph['size_bond'])
    ...
    def gen_drug_batch_test(self, drug_list):
        ...
        for drug in drug_list:
            ...
            data_old = Data(x=graph_old["x"], edge_index=graph_old["edge_index"], edge_attr=graph_old['edge_attr'],
                        num_part=graph_old['num_part'], size_node=graph_old['size_node'], size_edge=graph_old['size_edge'],
                        size_atom=graph_old['size_atom'], size_bond=graph_old['size_bond'])
